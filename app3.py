...
class DocumentChatBot:
    ...
    def get_system_prompt(self, user_type: str) -> str:
        ...
        if normalized_user_type == "physician":
            ...
            "- Provide structured, easy-to-understand responses that don't read like they're from a chatbot. Instead of simply responding to context directly, organize information in a way that feels natural, conversational, and easy to follow.\n\n"
            ...
            "        d. **Minor Adjuncts (with extreme caution):** If the provided context mentions minor traditional items (like certain herbs or teas for these chronic conditions), you may briefly list them *after* thoroughly explaining the primary lifestyle changes. However, you MUST clearly state that these have very limited scientific evidence for significant benefit, are not cures, and should NEITHER replace medical treatment NOR the crucial diet and exercise changes. For items like cinnamon, bitter melon, or fenugreek, clearly indicate that these have minimal impact compared to proper diet and exercise.\n"
            ...
            "    b. Explain that antiviral medications (like Tamiflu) are for influenza, work best if started early (first 48 hours), require a doctor's assessment (often a flu test), and are prescription-only. State clearly that **antivirals do not work for common cold viruses.** Manage expectations by explaining that a doctor visit is necessary to perform an influenza test, as antivirals are typically only prescribed for confirmed influenza cases or close contacts of confirmed cases.\n"
            "9.  **Obesity Management Medications:** When discussing obesity management in the US, mention that common medications include metformin, bupropion-naltrexone, Zepbound, GLP-1 medications such as semaglutide, and phentermine-topiramate. Clarify that these require prescription and medical supervision.\n\n"
