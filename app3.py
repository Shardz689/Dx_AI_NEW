...
class DocumentChatBot:
    ...
    def enhance_with_triage_detection(self, query, response_content):
        """Enhance family user responses with triage situation detection"""
        user_type = st.session_state.get("user_type", "family")
        if user_type != "family":
            return response_content
    
        triage_indicators = [
            "emergency", "urgent care", "primary care", "self-care",
            "call 911", "immediate medical attention", "see a doctor within"
        ]
        if any(indicator in response_content.lower() for indicator in triage_indicators):
            return response_content
    
        triage_prompt = (
            f"Analyze the following medical query and determine if it represents a triage situation:\n\n"
            f"QUERY: {query}\n\n"
            f"CURRENT RESPONSE: {response_content}\n\n"
            "If this is a triage situation, classify it according to these categories:\n"
            "1. Emergency (Call 911/Emergency Services immediately)\n"
            "2. Urgent Care (See a doctor within 24 hours)\n"
            "3. Primary Care (Schedule a regular appointment)\n"
            "4. Self-care (Can be managed at home)\n\n"
            "Provide only the triage category and a brief explanation. If it's not a triage situation, respond with 'NO_TRIAGE_NEEDED'."
        )
    
        try:
            triage_analysis = self.llm.invoke(triage_prompt, config={"max_output_tokens": 150})
            triage_text = triage_analysis.content
    ...
    def generate_response(self, query, conversation_history=None):
      ...
      if conversation_history:
          for user_msg, bot_msg in conversation_history:
              final_prompt.append({"role": "user", "content": user_msg})
              final_prompt.append({"role": "assistant", "content": bot_msg})
      ...
      if user_type == "family":
          result = self.enhance_with_triage_detection(query, result)
      return result      
...
def create_user_type_selector():
    if "user_type" not in st.session_state:
        st.session_state.user_type = "family"  # Default to family

    user_type = st.sidebar.radio(
        "Select User Type:",
        options=["family", "physician"],
        index=0 if st.session_state.user_type == "family" else 1,
        help="Select whether you are a family member or a physician"
    )

    if user_type != st.session_state.user_type:
        st.session_state.user_type = user_type
        st.session_state.messages = []  # Optionally clear chat history on user type change
